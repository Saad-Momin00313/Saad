...
class AIFinancialInsights:
    def __init__(self, api_key=None):
        """
        Initialize Groq client for financial insights
        ...
        if not self.api_key:
            raise ValueError(
                ...
                "Set it in .env file as GROQ_API_KEY or pass directly to constructor."
            )
    ...
    def analyze_spending_patterns(self, expenses_df):
        ...
        trends = {
            ...
            'monthly_trend': monthly_spending.to_dict('records')
        }
        return trends
    ...
    def generate_spending_insights(self, expenses_df):
        ...
        expense_hash = pd.util.hash_pandas_object(expenses_df).sum()
        ...
        try:
            chat_completion = self.client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional financial advisor providing actionable insights."
                    },
                    {"role": "user", "content": prompt}
                ],
                model="llama3-8b-8192"
